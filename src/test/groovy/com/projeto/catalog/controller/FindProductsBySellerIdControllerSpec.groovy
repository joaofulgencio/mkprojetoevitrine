...
class FindProductsBySellerIdControllerSpec extends Specification {
    ...
    def "Should return all products registered for a specific user"() {
        ...
        def sellerEmail = "joaooctf@gmail.com"
        ...
        1 * findProductsBySellerIdUseCase.execute(_ as String) >> {
            return [new Product("joaooctf@gmail.com", "Camisa", [new Image("image1.jpg")], "camisa maneira", 40),
                    new Product("joaooctf@gmail.com", "Camisa 2", [new Image("image1.jpg"), new Image("image2.jpg")], "camisa maneira 2", 20)]
        }


        then: "the request must be done with successful response"
        Gson gson = new Gson()
        def response = mockMvc.perform(get("/catalog/{sellerEmail}", sellerEmail).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn()
                .response
        ...
        responseObject[0].sellerEmail == "joaooctf@gmail.com"
